...
import { subscribeToPush, scheduleTaskNotification, onNotificationToast } from "@/utils/pushManager";
import { NotificationToastContainer, ToastPayload } from "@/components/NotificationToast";
...
export default function ClientComponent({ accessToken }: { accessToken: string }) {
  ...
  useEffect(() => {
    ...
    onNotificationToast((payload) => setToasts((prev) => [...prev, payload]));
  }, []);
              ...
              if (topEmotion && topEmotion !== lastTopEmotionRef.current) {
                ...
                fetchTasksForEmotion(topEmotion)
                  .then((tasks) => {
                    ...
                    if (tasks[0]) scheduleTaskNotification(tasks[0], 0);
                  })
                ...
                {!loadingTasks && suggestedTasks.map((task, idx) => (
                  <div
                    ...
                    onClick={async () => {
                      ...
                      scheduleTaskNotification(task, task.duration);
                      setChainedTasks([]);
                      ...
                      if (chained[0]) scheduleTaskNotification(chained[0], 0, 'Up next');
                    }}
                  ...
                  {chainedTasks.map((task, idx) => (
                    <div
                      ...
                      onClick={async () => {
                        ...
                        if (newChained[0]) scheduleTaskNotification(newChained[0], 0, 'Up next');
                      }}
                      className="relative cursor-pointer p-4 rounded-2xl text-white shadow-md flex flex-col items-center justify-center overflow-hidden break-words transition-all duration-500 z-10"
                      style={{
                        opacity: 0.5,
